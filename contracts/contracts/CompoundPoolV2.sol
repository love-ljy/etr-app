...
/**
 * @title CompoundPoolV2
 * @dev 复利池合约 V2
 * - 接收来自 StakingPool 的收益入账
 * - 每日按日化收益率计算复利
 * - 用户可随时提取或划转到余额
 * - 支持开关控制：是否允许用户直接存入 ETR
 */
contract CompoundPoolV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant DEFAULT_YIELD_RATE = 45;          // 默认日化 0.45% (基点)
    ...
    bool public allowDirectDeposit;   // 是否允许用户直接存入 ETR（复利开关）
    ...
    mapping(address => uint256) public totalDirectDeposited; // 累计直接存入金额（区分来源）
    ...
    modifier onlyStakingPool() {
        require(msg.sender == stakingPool, "CompoundPoolV2: only staking pool");
        _;
    }
    ...
    constructor(address _etrToken) {
        require(_etrToken != address(0), "CompoundPoolV2: ETR token is zero address");
        
        etrToken = IERC20(_etrToken);
        currentYieldRate = DEFAULT_YIELD_RATE;
        allowDirectDeposit = false; // 默认关闭直接存入
    }
    
    // ============ 核心函数 ============
    
    /**
     * @dev 存入奖励（由 StakingPool 调用）
     * @param user 用户地址
     * @param amount 奖励金额
     */
    function depositReward(address user, uint256 amount) 
        ...
        nonReentrant 
    {
        require(user != address(0), "CompoundPoolV2: user is zero address");
        require(amount > 0, "CompoundPoolV2: amount must be greater than 0");
    ...
    /**
     * @dev 用户直接存入 ETR 到复利池（受开关控制）
     * @param amount 存入金额
     */
    function depositETR(uint256 amount) external whenNotPaused nonReentrant {
        require(allowDirectDeposit, "CompoundPoolV2: direct deposit is disabled");
        require(msg.sender != address(0), "CompoundPoolV2: user is zero address");
        require(amount > 0, "CompoundPoolV2: amount must be greater than 0");
        ...
        totalDirectDeposited[msg.sender] += amount;
        lastCompoundTime[msg.sender] = block.timestamp;
        
        // 更新全局统计
        totalCompoundPool += amount;
        
        emit DirectDeposit(msg.sender, amount, compoundBalances[msg.sender], block.timestamp);
    }
    ...
    function claimCompound() external whenNotPaused nonReentrant returns (uint256 amount) {
        ...
        require(amount > 0, "CompoundPoolV2: no compound to claim");
    ...
    function transferToBalance(uint256 amount) external whenNotPaused nonReentrant {
        ...
        require(compoundBalances[msg.sender] >= amount, "CompoundPoolV2: insufficient compound balance");
    ...
    function getCompoundInfo(address user) external view returns (
        ...
        uint256 totalDirectDepositedAmount,
        ...
        totalDirectDepositedAmount = totalDirectDeposited[user];
    ...
    /**
     * @dev 设置 StakingPool 地址
     */
    function setStakingPool(address _stakingPool) external onlyOwner {
        require(_stakingPool != address(0), "CompoundPoolV2: staking pool is zero address");
    ...
    function updateYieldRate(uint256 newRate) external onlyOwner {
        require(newRate >= DEFAULT_MIN_YIELD_RATE && newRate <= DEFAULT_MAX_YIELD_RATE, 
            "CompoundPoolV2: rate out of range");
    ...
    /**
     * @dev 切换复利开关（控制是否支持用户直接存入 ETR）
     * @param enabled true=开启，false=关闭
     */
    function setDirectDepositEnabled(bool enabled) external onlyOwner {
        allowDirectDeposit = enabled;
        emit DirectDepositToggled(enabled, block.timestamp);
    }
    ...
    function emergencyWithdraw(address to, uint256 amount) external onlyOwner {
        require(to != address(0), "CompoundPoolV2: zero address");
